...
CV1=$(grep opes1 plumed.dat | awk '{print $3}' | cut -d "=" -f2 | cut -d "," -f1)
CV2=$(grep opes2 plumed.dat | awk '{print $3}' | cut -d "=" -f2 | cut -d "," -f2)
min1=-0.5
max1=2.5
min2=-1
max2=4.0
sigma1=$(tail -n1 KERNELS1 | awk '{printf "%.3f", $3}')
sigma2=$(tail -n1 KERNELS2 | awk '{printf "%.3f", $3}')
...
then
 echo "Reweighting done"
  ...
  if [ -f STATE1 ]
  then
    echo "STATE1 file exists"
  else
    python ~/Downloads/others/opes-metad/State_from_Kernels.py --kernels KERNELS1 --outfile STATE1
  fi
python ~/Downloads/others/opes-metad/FES_from_State.py  --state STATE1 --outfile fes-stat_square_sparse_walls_1.dat --temp $temp  --min " ${min1}"," $min2" --max ${max1},$max2 

  if [ -f STATE2 ]
  then
    echo "STATE2 file exists"
  else
    python ~/Downloads/others/opes-metad/State_from_Kernels.py --kernels KERNELS2 --outfile STATE2
  fi
python ~/Downloads/others/opes-metad/FES_from_State.py  --state STATE2 --outfile fes-stat_square_sparse_walls_2.dat --temp $temp  --min " $min2" --max $max2 
...
##STATES##
python ~/SHARED/GitHub/mio/convert/plot2d_reweight.py fes-stat_square_sparse_walls_1.dat $CV1 $CV2 
python ~/SHARED/GitHub/mio/convert/plot2d_reweight.py fes-stat_square_sparse_walls_2.dat $CV1 $CV2 
#python ~/SHARED/GitHub/mio/convert/find_CV_struct.py --input colvar_trj --CVs $CV1 $CV2 --val minima.dat --tol 0.1 0.1
