...
min2=-0.5
max2=3.5
sigma1=$(tail -n1 KERNELS | awk '{printf "%.3f", $5}')
sigma2=$(tail -n1 KERNELS | awk '{printf "%.3f", $6}')
temp=300
skip=50
...
python ~/SHARED/GitHub/mio/convert/plot_1D-2D_seaborn.py fes-rew_square_sparse.dat $CV1 $CV2 fes-rew_square_sparse_cv1.dat fes-rew_square_sparse_cv2.dat
