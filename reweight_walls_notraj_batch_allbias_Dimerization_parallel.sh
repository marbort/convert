...
CV1=$(grep opes: plumed.dat | awk '{print $3}' | cut -d "=" -f2 | cut -d "," -f2)
CV2=$(grep opes: plumed.dat | awk '{print $3}' | cut -d "=" -f2 | cut -d "," -f1)
min1=1.8
max1=4.0
min2=0.0
max2=8.0
...
then
 echo "Reweighting done"
  ...
  #
  python ~/Downloads/others/opes-metad/FES_from_Reweighting_parallel.py  --colvar $file --outfile fes-rew_square_sparse_walls.dat --sigma ${sigma1},$sigma2 --temp $temp --cv $CV1,$CV2 --bias $bias --min " ${min1}"," $min2" --max ${max1},$max2 --skiprows $skip
  if [ -f STATE ]
  then
...
python ~/SHARED/GitHub/mio/convert/plot2d.py --input fes-rew_square_sparse_walls.dat --xlab CV1 --ylab CV2 --max 200 
...
##STATES##
python ~/SHARED/GitHub/mio/convert/plot2d.py --input fes-stat_square_sparse_walls.dat --xlab CV1 --ylab CV2  --max 200
