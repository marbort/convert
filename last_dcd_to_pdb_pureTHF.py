...
def lastdcd_to_qmmm(topo,trj,sel):
    ...
    with open(f"residues_{val}.dat",'w') as ofile:
        for resid in QMmm_residues:
                ofile.write(f"{resid} ")
    with open(f"residues_CPMD_{val}.dat",'w') as ofile:
        for resid in QMmm_residues:
                ofile.write(f"add resid is {resid}\n")
...
for input in inputs:
    Cl1_pos,Cl2_pos,avg=get_avg_Cl(1,2,topo,input)
    Cl_avg_pos=" ".join([str(x) for x in avg])
    #sel=f"byres point {Cl_avg_pos} 8.0"
    sel=f"resid 1 2 or (byres point {Cl_avg_pos} {radius})"
    print(input)
