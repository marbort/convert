...
def extract_cp2k_coords(file, kind,argsoffset):
    ...
    print("Extracting {:d} frames".format(nframes//argsoffset))
    traj_ener = [np.append(traj_ener, float(lines[argsoffset*k*(natom+2)+1].split()[8].rstrip())*hartree_to_ev)
                 for k in range(nframes//argsoffset)]

    time = [float(lines[argsoffset*k*(natom+2)+1].split()[5].strip(','))
            for k in range(max(nframes//argsoffset,1))]
    print(time[0])

    # group val in an array for each frame
    val = [[float(lines[argsoffset*k*(natom+2)+i+2].split()[l])
           for i in range(natom) for l in range(1, 4)]
           for k in range(nframes//argsoffset)]

    # group val in an array for each element
    val_sch = [[[float(lines[argsoffset*k*(natom+2)+i+2].split()[l])
...
def savenpy(kind, val, traj_energy, types_map, types, dir, set, box, numbers):
    ...
    if box:
        box_0 = [[box[0], 0., 0., 0., box[1], 0., 0., 0., box[2]]]
...
def main(evalf: bool, 
    ...
    kind = "coord"
    savenpy(kind, crds_traj, traj_energy_traj,
            types_map, types, dir, set, box_traj,numbers)
...
if __name__ == "__main__":
   # stuff only to run when not called via 'import' here
    ...
    args = parser.parse_args()
    print(args.feval)
    main(args.feval,args.offset,args.box)