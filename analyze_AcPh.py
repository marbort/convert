...
def RDF_COM_volume(gA,gB,Universe,binsize,max):
    ...
    gA_coms=[groupA.center_of_mass(pbc=True,compound='residues') for ts in Universe.trajectory]
    gB_coms=[groupB.center_of_mass(pbc=True,compound='residues') for ts in Universe.trajectory]
    if groupA == groupB:
        com_arr  = [distances.self_distance_array(x) for x in gA_coms]
        com_dist = [np.tile(x,2) for x in com_arr]
    else:
        com_arr=[distances.distance_array(x,gB_coms[i]) for i,x in enumerate(gA_coms)]
    ...
    return(bins,sum_norm,avg_vol)

def RDF_COM(gA,gB,Universe,binsize,max):
    groupA=Universe.select_atoms(gA)
    groupB=Universe.select_atoms(gB)
    gA_coms=[groupA.center_of_mass(pbc=True,compound='residues') for ts in Universe.trajectory]
    gB_coms=[groupB.center_of_mass(pbc=True,compound='residues') for ts in Universe.trajectory]
    ...
    for i in hists:
        sum_tmp=sum_tmp+i[0] 
    sum_norm=(sum_tmp/(4/3 * np.pi * (hists[0][1][1:]**3 - hists[0][1][:-1]**3)*(len(gA_coms)-1)))
...
#for i in paths:
u = mda.Universe(root+"interface_BIG_MOD_AcPhOnly.parm7",root+"npt_iso_1-2.xtc")
print(len(u.trajectory))
#%%
mols=["CHL","GCL","THF","Clm","ACP"]
labs={"CHL":"Choline","GCL":"Glycerol","THF":"THF",'Clm':'Chloride','AcPh':'Acetophenone'}
clrs=['#F22727','#12ab07','#072dab','#ffa6a7','#ff8533','#8A2BE2','#000000']
clrs_dict={"CHL":'#F22727',"GCL":'#12ab07',"THF":'#072dab',
           "IPM":'#4b68c9',"IMC":'#8393c7',"MG":'#ffa6a7',"ACT":'#ff8533',"Clm":'#8A2BE2','AcPh':'#ff8533'}
...
ACT = u.select_atoms('resname ACT')
ACP = u.select_atoms('resname ACP')
...
Clm_coms=[Clm.center_of_mass(pbc=True,compound='residues') for ts in u.trajectory]
ACP_coms=[ACP.center_of_mass(pbc=True,compound='residues') for ts in u.trajectory]
...
ACP_GCL_com_arr=[distances.distance_array(x,GCL_coms[i]) for i,x in enumerate(CHL_coms)]
