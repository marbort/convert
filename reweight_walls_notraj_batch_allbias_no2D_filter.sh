...
then
 echo "Reweighting done"
  ...
  python ~/SHARED/GitHub/mio/convert/filter_colvar.py colvar
  ...
  #
  python ~/Downloads/others/opes-metad/FES_from_Reweighting.py  --colvar $file --outfile fes-rew_square_sparse_cv2_walls.dat --sigma $sigma2 --temp $temp --cv $CV2 --bias $bias --min " $min2" --max $max2 --skiprows $skip --stride $stride_conv
  #
  if [ -f STATE ]
  then
