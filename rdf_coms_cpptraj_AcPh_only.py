...
def coord_number2(x,y,dist_min,dist_max):
    ...
    return(CN_all)
#%%
root="/home/marco/SHARED/RATIO/WP4/MD/MOD-FRC/BIG/AcPh/"
...
# %%
resnumb={"CHL":834,"Clm":834,"GCL":1668,"THF":3337,"ACP":45}
...
for i in coms2:
    with open(i,'r') as ifile:
        ...
        for line in lines[1:]:
                x.append(float(line.split()[0]))
                y.append(float(line.split()[1]))
                y_int.append(float(line.split()[2]))
                y_raw.append(float(line.split()[3]))
        y_max_index=y.index(max(y))
        x_max=x[y_max_index]
        y_min=find_peaks([-x for x in y],width=2)
        x_min=x[y_min[0][0]]
        y_norm=[k/(4/3*np.pi*((x[i]+bin_size/2)**3-(x[i]-bin_size/2)**3)*lgth) for i,k in enumerate(y_raw)]
        y_norm_last=[k/y_norm[-1] for k in y_norm]
    

    
           
        
    RDFs2[name.split('_')[0]]={'name':'_'.join(name.split('_')[1:]),'RDF':[x,y,y_norm,y_norm_last,y_raw],'r_max':x_max,'r_min':x_min,'minima':y_min,'maxima':y_max_index,'#_part':resnumb[res]}
...
for i,item in enumerate(RDFs2):
    plt.plot(RDFs2[item]['RDF'][0], RDFs2[item]['RDF'][1],color=clrs[i],label=RDFs2[item]['name'])
plt.legend()    
for i,item in enumerate(RDFs2):
    plt.scatter(RDFs2[item]['r_max'],RDFs2[item]['RDF'][1][RDFs2[item]['maxima']],color=clrs[i])
    plt.scatter(RDFs2[item]['r_min'],RDFs2[item]['RDF'][1][RDFs2[item]['minima'][0][0]],color=clrs[i])

plt.title("COM-RDFs")
plt.figure(figsize=(10,10),dpi=150)
for i,item in enumerate(RDFs2):
    plt.plot(RDFs2[item]['RDF'][0], RDFs2[item]['RDF'][2],color=clrs[i],label=RDFs2[item]['name'])
plt.legend() 

plt.figure(figsize=(10,10),dpi=150)
for i,item in enumerate(RDFs2):
    plt.plot(RDFs2[item]['RDF'][0], RDFs2[item]['RDF'][3],color=clrs[i],label=RDFs2[item]['name'])
plt.legend()
...
#%% PI CATION
dist_path=glob.glob(root+'*dist*')
ang_path=glob.glob(root+'*angle*')
angles={}
dists={}
for i in dist_path:
    with open(i,'r') as ifile:
    ...
    frames=[float(x.split()[0]) for x in lines[1:]]
    ds=[float(x.split()[1]) for x in lines[1:]]
    dists[name]=[frames,ds]
...
for i in ang_path:
    ...
    frames=[float(x.split()[0]) for x in lines[1:]]
    ds=[float(x.split()[1]) for x in lines[1:]]
    angles[name]=[frames,ds]

for i in angles:
    print("Average angle for {} = {:>5.1f}".format(i,np.mean(angles[i][1])))

for i in dists:
    print("Average distance for {} = {:>5.1f}".format(i,np.mean(dists[i][1])))
...
plt.legend(legd)
for i,item in enumerate(RDFs3):  
    plt.scatter(RDFs3[item]['r_max'],RDFs3[item]['RDF'][1][RDFs3[item]['maxima']],color=clrs[i])
